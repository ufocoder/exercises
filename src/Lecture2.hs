...
removeAt idx list = removeAt_ 0 list []
  where
    removeAt_ _ [] list_ = (Nothing, list_)
    removeAt_ idx_ (x : xs) list_
        | idx == idx_   = (Just x, list_ ++ xs)
        | otherwise     = removeAt_ (idx_ + 1) xs (list_ ++ [x])
...
mergeSort xs = 
  let
      n = length xs
      leftPart = take (div n 2) xs
      rightPart = drop (div n 2) xs
  in
    merge (mergeSort leftPart) (mergeSort rightPart)
...
eval variables expression = 
  case expression of 
    Var variable -> 
       case lookup variable variables of
    ...
    Add a b -> evalSum (eval variables a) (eval variables b)


evalSum :: Either EvalError Int -> Either EvalError Int -> Either EvalError Int
evalSum (Left x) _ = Left x
evalSum _ (Left x) = Left x
evalSum (Right x) (Right y) = Right (x + y)
...
constantFolding expr =
  case expr of 
    ...
    Add expr1 expr2 -> 
      Add (constantFolding expr1) (constantFolding expr2)
